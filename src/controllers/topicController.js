...
const createTopic = async (req, res) => {
  try {
    ...
    const topic = await TopicModel.findOne({ name: name }, { _id: 0 });
...
const getTopic = async (req, res) => {
  try {
    const topic = await TopicModel.findById(req.params.id, { _id: 0 });
...
const getTopics = async (req, res) => {
  try {
    const topics = await TopicModel.find({}, { _id: 0 });
    if (topics.length === 0)
...
const updateTopic = async (req, res) => {
  try {
    ...
    const topicUpdated = await TopicModel.findById(req.params.id, { _id: 0 });
    res
      .status(200)
